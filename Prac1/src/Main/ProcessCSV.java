...
public class ProcessCSV {
	...
	/**
	 * Metodo constructor
	 * @param filePath El parametro filePath es la ruta del archivo de entrada
	 */
	...
	/**
	 * El metodo que lee los datos del archivo de entrada y
	 * guarda las informaciones procesadas al archivo de
	 * salida (node.csv y edge.csv)
	 */
		...
		while(line != null) {
			...
			for(int i = 0; i < movies.length; i++){
				...
				for(int j = i+1; j < movies.length; j++){
					...
					// guardamos la arista de los dos peliculas en la salida de aristas
					saveLine.clear();
					saveLine.add(String.valueOf(movieMap.get(movies[i])));
					saveLine.add(String.valueOf(movieMap.get(movies[j])));
					saveLine.add("undirected");
					this.edgeWriter.saveLine(saveLine);
